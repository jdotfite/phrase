...
$scriptFile = Split-Path $scriptName -Leaf

Write-Host "Starting analysis of $currentPath"
Write-Host "Excluding folders: $($excludedPaths -join ', ')"
Write-Host "Excluding files: $scriptFile, code-base.ps1, file-structure.ps1"
...
$allFiles = Get-ChildItem -Path $currentPath -Recurse -File | Where-Object {
    ...
    if ($_.Name -eq $scriptFile -or
        $_.Name -eq "code-base.ps1" -or
        $_.Name -eq "file-structure.ps1" -or
        $_.Name -eq $outputFile -or
        $_.Name.EndsWith(".tmp")) {
        $exclude = $true
    }
...
$allFiles | Where-Object {
    $_.Extension -notmatch '\.(exe|dll|pdb|obj|bin|cache|jpg|jpeg|png|gif|bmp|ico|mp3|mp4|zip|rar|7z)$' -and $_.Length -lt 5MB
} | ForEach-Object {
    $fileCounter++
